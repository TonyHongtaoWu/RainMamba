...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            lq_folder='../data/Video_rain_synthesis_train/Rain',
            gt_folder='../data/Video_rain_synthesis_train/GT',
            pipeline=[
                dict(
    ...
    test=dict(
        type='SRFolderMultipleGTDataset',
        lq_folder="../data/RainSynAll100/test/Rain",
        gt_folder="../data/RainSynAll100/test/GT",
        pipeline=[
            dict(
                type='GenerateSegmentIndices',
                start_idx=1,
...
log_config = dict(
    interval=1000, hooks=[dict(type='TextLoggerHook', by_epoch=False), dict(type='TensorboardLoggerHook', log_dir='../experiment/logs/RainSynAll100/', by_epoch=False)])
visual_config = None
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = '../experiment/RainSynAll100/'
load_from = None
resume_from = None
workflow = [('train', 1)]
