    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            if self.forward_ensemble is not None:
                ...
                hilbert_curve_large_scale = torch.load('./Hilbert/hilbert_curve_large_scale.pt', map_location=device)
                hilbert_curve_small_scale = torch.load('./Hilbert/hilbert_curve_small_scale.pt', map_location=device)
...
def test_video(lq, model, hilbert_curve64, hilbert_curve128):
        '''test the video as a whole or as clips (divided temporally). '''

        tile = [7, 128, 128]
        tile_overlap = [6, 40, 40]
        scale = 1
        window_size = [2, 8, 8]
        ...
        if num_frame_testing:
            ...
            for d_idx in d_idx_list:
                ...
                out_clip = test_clip(lq_clip, model, hilbert_curve64, hilbert_curve128)
...
def test_clip(lq, model, hilbert_curve64, hilbert_curve128):
    ...
    if size_patch_testing:
        ...
        for h_idx in h_idx_list:
            for w_idx in w_idx_list:
                ...
                out_patch = model(in_patch, hilbert_curve64, hilbert_curve128).detach().cpu()
        ...
        output = model(lq, hilbert_curve64, hilbert_curve128).detach().cpu()
