...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            lq_folder="../data/water/rainy_vid",
            gt_folder='../data/water/clean_vid',
            pipeline=[
                dict(
    ...
    test=dict(
        type='SRFolderMultipleGTDataset',
        lq_folder='../data/water/test/syn/rainy_vid',
        gt_folder='../data/water/test/syn/clean_vid',
        pipeline=[
            dict(
                type='GenerateSegmentIndices',
                start_idx=0,
...
log_config = dict(
    interval=1000, hooks=[dict(type='TextLoggerHook', by_epoch=False), dict(type='TensorboardLoggerHook', log_dir='../experiment/logs/waterdrop/', by_epoch=False)])
visual_config = None
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = '../experiment/waterdrop/'
load_from = None
resume_from = None
workflow = [('train', 1)]
